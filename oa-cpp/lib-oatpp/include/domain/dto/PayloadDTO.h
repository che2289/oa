...
public:
	...
	std::string getXsequence() const { return xsequence; }
	void setXsequence(std::string val) { xsequence = val; }
	...
	void propToJwt(T* obj) const
	{
		...
		obj->add_claim("id", id);
		// TIP：新增字段在后面补充即可
		obj->add_claim("xsequence", xsequence);
	}
	...
	void propToPayload(jwt::jwt_object* obj)
	{
		...
		// 转换用户序列号
		setXsequence(payload.get_claim_value<std::string>("xsequence"));
	}
};
