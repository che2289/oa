...
Pinyin::Pinyin()
{
	...
	size = sizeof(t2) / sizeof(t2[0]);
	code_qw=new int[size];
	for (int i = 0; i < size; i++)
	{
		code_qw[i] = t2[i];
	}
...
void Pinyin::GetPinYin(string ChineseStr, string& PinYin)
{
	// 获取一个指向字符串内容的const char*指针
	const char* cStr = ChineseStr.c_str();
	// 通过类型转换将其转换为unsigned char*指针
	unsigned char* Chinese = reinterpret_cast<unsigned char*>(const_cast<char*>(cStr));
...
void Pinyin::GetOtherCodePinYin(int nCode, string& strValue)
{
	switch (nCode)
	{
